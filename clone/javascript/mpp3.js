...
async function displayalbum() {
    try {
        const response = await fetch("http://127.0.0.1/clone/songs");
        ...
        let cardsHTML = ''; // Create an empty string to store HTML
        
        for (let index = 0; index < anchorsArray.length; index++) {
            const anchor = anchorsArray[index];
            if (anchor.href.includes("/songs")) {
                const folder = anchor.href.split("/").slice(-2)[0];
                const infoResponse = await fetch(`http://127.0.0.1/clone/songs/${folder}/info.json`);
                const infoJson = await infoResponse.json();
                // Concatenate HTML strings instead of directly appending
                cardsHTML += `<div data-folder="${folder}" class="card">
                    <div class="playbutton">
                        <img src="svg/play.svg" alt=""/>1
                    </div>
                    <img src="songs/${folder}/cover.jpg" alt=""/>
                    <h2>${infoJson.title}</h2>
        ...
        // Append the concatenated HTML after the loop
        cardContainer.innerHTML += cardsHTML;

        Array.from(document.getElementsByClassName("card")).forEach(card => {
            card.addEventListener("click", async () => {
                console.log("Fetching Songs");
                // Assuming getsongs and playmusic are defined elsewhere
...
/*async function displayalbum(){
    ...
    let array=Array.from(anchors)
        ...
        if(e.href.includes("/songs")){
            ...
            console.log(response)
            cardcontainer.innerHTML=cardcontainer.innerHTML + `<div data-folder="english" class="card">
                 <div class="playbutton">
